...
# from context.config import get_config
# config = get_config()
# processes_path = config.get("plugin", "processes")
...
# # 使用 Gunicorn 启动应用
# import subprocess
# subprocess.run(["gunicorn", "-w", "4", "-b", "0.0.0.0:5000", "main:main"])


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run Flask app with specific configuration."
    )
    parser.add_argument(
        "--deploy_mode", help="Deployment mode (e.g., single, distributed)"
    )
